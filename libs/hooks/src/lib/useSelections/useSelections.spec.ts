...
const setup = <T>(items: T[], defaultSelected?: T[]) =>
  renderHook(() => useSelections(items, defaultSelected));
...
describe('useSelections', () => {
  ...
  it('selectAll and unSelectAll should work correct', () => {
    const { result } = setup(data);
    ...
    expect(result.current.selected).toEqual([1, 2, 3, 4, 5, 6, 7, 8, 9]);
  ...
  it('toggleAll should work correct', () => {
    const { result } = setup(data);
  ...
  it('setSelected should work correct', () => {
    const { result } = setup(data);
