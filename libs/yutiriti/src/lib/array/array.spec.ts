...
describe('array module', () => {
  describe('group function', () => {
    test('groups by provided attribute', () => {
      ...
      expect(groups['a']?.length).toEqual(2);
      expect(groups['b']?.length).toEqual(2);
      expect(groups['c']?.length).toEqual(1);
      expect(groups['c']?.[0].word).toEqual('ok');
    });
  });
  ...
  describe('replace function', () => {
    ...
    test('returns copy of list with replaced item', () => {
      ...
      expect(result[0].game).toEqual('x');
      expect(list[1].game).toEqual('b');
    });
    test('returns copy of list without changing', () => {
      ...
      expect(result[0].game).toEqual('a');
      expect(list[1].game).toEqual('b');
    });
  });
  ...
  describe('objectify function', () => {
    ...
    test('returns correct map of values', () => {
      ...
      expect(result['a'].word).toEqual('hello');
      expect(result['b'].word).toEqual('bye');
    });
  ...
  describe('unique function', () => {
    ...
    test('uses key fn to correctly remove duplicate items', () => {
      ...
      expect(a.word).toEqual('hello');
      expect(b.id).toEqual('b');
      expect(b.word).toEqual('oh');
      expect(c.id).toEqual('c');
      expect(c.word).toEqual('yolo');
    });
  });
  ...
  describe('fork function', () => {
    ...
    test('returns correctly forked list', () => {
      ...
      const [r, s] = xs;
      expect(r.name).toEqual('ray');
      expect(s.name).toEqual('sara');
      const [b, m] = ys;
      expect(b.name).toEqual('bo');
      expect(m.name).toEqual('mary');
    });
  });
  ...
  describe('merge function', () => {
    ...
    test('returns correctly mergeped lists', () => {
      ...
      expect(result[0].group).toEqual('XXX');
      expect(result[1].group).toEqual('X');
      expect(result[2].group).toEqual('Y');
      expect(result[3].group).toEqual('YYY');
    });
  });
  ...
  describe('alphabetical function', () => {
    test('uses getter', () => {
      ...
      expect(result[0].name).toEqual('AJ');
      expect(result[1].name).toEqual('Cynthia');
      expect(result[2].name).toEqual('Leo');
    });
    test('uses descending order', () => {
      const list = [{ name: 'Leo' }, { name: 'AJ' }, { name: 'Cynthia' }];
      const result = _.alphabetical(list, (i) => i.name, 'desc');
      expect(result[0].name).toEqual('Leo');
      expect(result[1].name).toEqual('Cynthia');
      expect(result[2].name).toEqual('AJ');
    });
    test('gracefully handles null input list', () => {
